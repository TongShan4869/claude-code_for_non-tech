...
因为 Markdown 就是纯文本，你可以用任何工具查看和编辑。像 [Obsidian](https://obsidian.md/) 这样的笔记应用能漂亮地渲染 Markdown，还支持双向链接、图谱视图和实时预览，是搭配 Claude Code 使用的绝佳可视化伙伴。
...
下面是一个真实对话的样子。假设你有一个文件夹，里面存了过去一个月的十几份会议纪要：

> **你：** 所有会议纪要里的主要待办事项有哪些？
>
> **Claude：** 我来读一下你的会议纪要。*（读取全部 12 个文件）* 以下是我在你的会议纪要中找到的未完成待办事项：
> - **Henderson 提案：** Sarah 需要在周五前发送修改后的预算（来自 3 月 12 日会议）
> - **Q2 规划：** 你承诺起草时间表，尚未完成（来自 3 月 18 日会议）
> - **客户入职流程：** 等待法务审核新合同模板（来自 3 月 24 日会议）
> - *（还有 3 项...）*
>
> **你：** 把分配给我的那些单独拉出来，放到一个新文件 my-action-items.md 里
>
> **Claude：** 好了。我已经创建了 `my-action-items.md`，包含你的 4 项待办事项，按截止日期排列。

就这样。不用在文件之间复制粘贴，不用自己翻文档。Claude 读完了所有内容，找到了关键信息，并创建了你要求的输出。
...
Claude 很能干，但不是万无一失的。以下是最常见的问题及应对方式。

**Claude 改错了文件或做了不想要的修改。** 输入 `/undo`。这会撤销 Claude 的上一步操作。你也可以用自然语言说"撤销刚才的操作"或"恢复上一次的修改"。如果你一直在用 git 做提交，还可以回退更远："恢复到上次提交的版本。"

**Claude 好像糊涂了，回复质量在下降。** 长时间的会话会积累大量上下文，有时会让思路变混乱。输入 `/clear` 清空对话，从头开始。你的文件不受影响，只有对话被重置。如果不想完全清空，试试 `/compact`，它会压缩历史但不会完全丢弃。

**Claude 理解错了你的意思。** 直接纠正它，就像跟同事说话一样。"不是，我说的是 Q2 的报告，不是 Q1 的。"或者"太正式了，用轻松的语气重写一遍。"不需要从头来过。Claude 会根据你在同一个会话中的反馈进行调整。

**Claude 找不到你知道存在的文件。** 它可能在找错地方。说得具体一些："读取 clients 文件夹里那个叫 proposal-v2.md 的文件。"你也可以说"列出这个文件夹里所有文件"来看看 Claude 能看到什么。

**你想在接受修改之前看看 Claude 改了什么。** 说"给我看看你刚才改了什么"或者"你修改了哪些内容？" Claude 会精确展示哪些行被添加、删除或修改了。如果你用了 git，"给我看 diff"能展示所有未提交的更改全貌。

**出了问题，不确定发生了什么。** 别慌。如果你一直在用 git，一切都是可恢复的。说"给我看 git 日志"查看最近的快照，然后"恢复到（那个提交）"就能回到已知的正常状态。即使没用 git，`/undo` 也能逐步回退 Claude 最近的操作。

总体原则：Claude 做了你不喜欢的事，直接说。会话感觉不对劲，就开新的。提交过的工作，随时能找回来。
